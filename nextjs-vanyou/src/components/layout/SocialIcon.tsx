...
/**
 * 根据 URL 或 label 渲染对应社交图标，仅内联 SVG，不引入图标库。
 */
export type SocialKind = "instagram" | "youtube" | "x" | "facebook" | "linkedin" | "generic";

function getKindFromUrl(url: string | null): SocialKind {
...
export function SocialIcon({ url, label, className }: SocialIconProps) {
  const kind = getKindFromUrl(url);
  return <span className={className}>{icons[kind]}</span>;
}
